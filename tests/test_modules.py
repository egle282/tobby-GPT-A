...
class DummyBot:
    """ Минимальный мок-бот для тестов: просто записывает вызовы. """
    def __init__(self):
...
def test_qr_scanner():
    ...
    dummy_msg.content_type = 'photo'
    # Без реального фото вернет False/True в зависимости от try/except
    assert mod.handle(dummy_msg) in [True, False]
...
def test_ai_handler():
    ...
    # Без API-ключа вернёт False, с ключом — True.
    assert mod.handle(dummy_msg) in [True, False]
