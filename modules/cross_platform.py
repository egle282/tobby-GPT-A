...
class CrossPlatform:
    def __init__(self, bot, is_enabled_cb):
        ...
        self.await_info = set()  # Пользователи, ожидающие ввода платформы
    ...
    def handle(self, msg):
        ...
        if msg.from_user.id in self.await_info:
            ...
            supported = {"web": "Web", "ios": "iOS", "android": "Android"}
            if platform in supported:
                self.bot.send_message(
                    msg.chat.id,
                    f"Инструкции для {supported[platform]} платформы: (здесь будет текст для {supported[platform]})"
                )
                self.await_info.remove(msg.from_user.id)
            else:
                self.bot.send_message(
                    msg.chat.id,
                    "Платформа не распознана. Доступно: Web, iOS, Android."
                )
            return True
